            ...
            <tbody>
                ...
                @foreach ($barangs as $barang)
                    <tr>
                        ...
                        <td>{{ $barang->status }}</td>
                        <td>
                            <form action="/dashboard/vbarangs/{{ $barang->slug }}" method="post" class="d-inline">
                                @method('put')
                                <input type="text" class="form-control" id="status" name="status" hidden
                                    value="Belum di Posting" value="{{ old('status') }}">
                                @csrf
                                <button class="badge bg-warning border-0"
                                    onclick="return confirm('Apakah Anda yakin barang sudah dibayar?')"><span
